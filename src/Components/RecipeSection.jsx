      ...
      {selected && (
        <div className="fixed inset-0 bg-black bg-opacity-50 flex justify-center items-center z-50">
          <div className="bg-white p-6 rounded-lg shadow-lg max-w-md w-full relative animate-fadeIn">
            ...
            >
              <i className="fa-solid text-2xl fa-circle-xmark"></i>
            </button>
            <h3 className="text-2xl font-bold text-green-600 mb-3">{selected.title}</h3>
            <img src={`images/${selected.image}`} alt={selected.title} className="rounded mb-4" />
            ...
            <p className="text-gray-700 mb-2">
              <strong>Steps:</strong> {selected.steps.map((step, i) => (
                <li key={i} className="ml-4 list-disc">{step}</li>
              ))}
            </p>
